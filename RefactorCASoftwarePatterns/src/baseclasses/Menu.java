...
public class Menu extends JFrame{
			...
			public void actionPerformed(ActionEvent ae) {
			    ...
			    else
			    {
				    ...
				    if(account.equals("Current Account"))
				    {
				    	...
				    	CurrentAccount current = new CurrentAccount(atm, number, balance, transactionList);
	...
	{	
		...
		lodgementButton.addActionListener(new ActionListener(  ) {
			...
			{
				int count = 3;
				int checkPin = ((CurrentAccount) acc).getAtm().getPin();
				loop = true;
				...
				while(loop)
				{
					if(count == 0)
					{
						...
						((CurrentAccount) acc).getAtm().setValid(false);
						customer(e); 
						loop = false;
						on = false;
					}
